...
gboolean build_program_cb(GtkWidget *widget, gpointer data)
{
	...
	char *string = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(text->programBuffer), &start, &end, TRUE);
	struct VString *program = new(VString, string, utf8_char_number(string, -1));
	g_free(string);
	...
	delete_empty_lines(program);

	if(output){
		variables->lastAddress = LAST_RAM_ADDRESS;
		output = variables_array_init(program, variables);
	}
	...
	if(output)
		output = labels_array_init(program, labels); 

	if(output)
		output = label_variable_array_conflict_check(variables, labels);

	if(output)
		output = variables_array_replace(program, variables);

	if(output)
		output = org_array_init(program, orgs);
