...
static const struct vstring_vtable vtable = 
{
   ...
   vstring_get_substring,
   vstring_get_substring_in_line,
...
void *vstring_clone(const void *_self)
{
	const struct VString *self = _self;
	assert(self->base.class == VString);

	return new(VString, self->base.array, self->charNumber);
}
...
uint64_t vstring_get_substring(const void *_self, void *_substring, uint64_t pos)
{
	const struct VString *self = _self;
	struct VString *substring = _substring;
	bool returnValue = false;
	char character[BYTE_LEN_UTF8] = {0};
	uint64_t j;
	uint64_t i;
	...
	for (j = pos; vstring_get_element_at(self, character, j); ++j) {
		...
		if (!g_unichar_isspace(unicharacter)){
			...
			vstring_erase_at(substring, 0, substring->charNumber);
			vstring_insert_vector_at(substring, 0, self, j, i - j);
			break;
		}
	}
...
uint64_t vstring_get_substring_in_line(const void *_self, void *_substring, uint64_t pos)
{
	const struct VString *self = _self;
	struct VString *substring = _substring;
	char character[BYTE_LEN_UTF8] = {0};
	...
	for (j = pos; vstring_get_element_at(self, character, j); ++j) {
		...
		if (unicharacter == NEW_LINE)
			...
			return vstring_get_substring(self, substring, j);
	}
...
int64_t vstring_string_to_hex(const void *_self)
{
	const struct VString *self = _self;
	const char *string = self->base.array;
...
uint64_t get_substring(const void *_self, void *_substring, uint64_t pos)
{
	const struct VString *self = _self;
	struct VString *substring = _substring;
	...
	return ((const struct vstring_vtable *)self->base.vtable)->vstring_get_substring(self, substring, pos);
}

uint64_t get_substring_in_line(const void *_self, void *_substring, uint64_t pos)
{
	const struct VString *self = _self;
	struct VString *substring = _substring;
	...
	return ((const struct vstring_vtable *)self->base.vtable)->vstring_get_substring_in_line(self, substring, pos);
}
...
int64_t string_to_hex(const void *_self)
{
	const struct VString *self = _self;
